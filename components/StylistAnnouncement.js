...
const AnnouncementBanner = ({ announcement }) => {
  ...
  return (
    <div
      className={`bg-cover bg-center text-white py-24 px-8 rounded-lg ${
        !title && !body ? 'bg-opacity-0' : ''
      }`}
      style={{ backgroundImage: `url(${imageUrl})` }}
    >
      {title || body ? (
        <div className="bg-black bg-opacity-50 flex items-center justify-center py-24 px-8 rounded-lg">
